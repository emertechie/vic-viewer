# Next Tasks

- [ ] Add light and dark mode toggle. Implement light mode if not supported already. Swith to using Geist Mono font. And fix the Run Query button style in dark mode - it always looks like it's disabled currently.
- [ ] If all text removed from search bar and return pressed: default to "*", and execute the search
- [ ] For Log details slide out: use a copy icon button, not button with "Copy" text. Also make the whole slide out panel a bit wider - 500px.
- [ ] Below the "Raw JSON" field in the "Log Details" slide out, add a toggle to wrap text
- [ ] Track if there are any unapplied changes to the current query (e.g. the user has updated the LogsQL text but not pressed return, or they have changed a datetime value and not pressed apply, etc) and if so, give some indication to the user that there are unapplied changes. Perhaps an indicator message or icon. Whatever you think will work best, and is most common in these kind of log viewer UIs.

