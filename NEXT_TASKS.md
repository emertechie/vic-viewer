...
- [ ] Two small changes: Add a clear / "x" button to the search bar so that pressing it will replace any existing search with "\*". It should not auto-execute the query though. Then, add some background colour to the log table's header (like a light grey in light mode), to help differentiate it.
